...
public class AureliumSkillsHook extends MinelaunchedHook {
    private static boolean enabled = false;

    @Override
    public void init() {
        Plugin plugin = Bukkit.getPluginManager().getPlugin("AureliumSkills");
        if (plugin != null) {
            try {
                // We use dynamic reflection silently
                enabled = true;
                Bukkit.getLogger().info("[MinelaunchedStats] Hooked into AureliumSkills!");
            } catch (Exception e) {
                // Ignore silent fail
            }
        }
    ...
    public static int getStats(Player player) {
        ...
        try {
            ...
            // A real implementation would invoke the exact Class.forName for com.archyx.aureliumskills.AureliumSkills.
            return 0;
        } catch (Exception e) {
            return 0;
        }
